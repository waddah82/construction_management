...
def get_exchange_rate(filters, company_currency):
    ...
    if not exchange_rate:
        frappe.throw(_("mmmmmmmmm "
                      "nnnnnnn").format(
            company_currency, target_currency))
...
def prepare_data(accounts, filters, total_row, parent_children_map, company_currency):
    ...
    exchange_rate = get_exchange_rate(filters, company_currency)
    ...
    for d in accounts:
        ...
        for key in value_fields:
            ...
            row[key] = flt(d.get(key, 0.0) / exchange_rate, 3)
    ...
    total_row_copy["currency"] = filters.get('currency') or company_currency
    total_row_copy["account_name"] = _("Total")
    
    for key in value_fields:
        total_row_copy[key] = flt(total_row[key] / exchange_rate, 3)
